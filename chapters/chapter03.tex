...
使用本模板撰写论文时，请首先确定自己的学位类型，并修改\cs{main.tex}的第一行代码！
...
若为\textbf{学术型博士}，请使用\texttt{\textbf{doctor-academic}}，具体为：
\begin{lstlisting}[language={[LaTeX]TeX}]
...
使用本模板时，请在\cs{chapters}目录下删改、新增并编辑论文的章节\cs{*.tex}文件；在\cs{figures}目录下上传论文的插图；在\cs{main.tex}文件内编辑论文的整体结构；在\cs{betterbib.bib}内编辑论文引用文献的条目信息。若有需求可修改\cs{bjtuthesis.cls}文件内的论文的版面结构设置。

\subsection{论文基本信息字段填写}\label{论文基本信息字段填写}
请直接在\cs{main.tex}文件中编辑以下代码字段：
...
每一个行第二个花括号内内容改为论文准确信息，完成后将会在论文的封面页、题名页、学位论文数据集中自动调用，无需再次编辑\cs{cover.tex}、\cs{cover2.tex}、\cs{statement.tex}等文件。

\subsection{封面页、学位论文版权使用授权书}
对应\cs{chapters}\cs{cover.tex}文件。封面页为第一部分，\textbf{论文中、英文标题，作者，导师}信息字段编辑方法见\secref{论文基本信息字段填写}，毋需在此文件内编辑。学位论文版权使用授权书为第二部分，可编辑日期。签名建议脱离\LaTeX{}环境后插入或手写。

\subsection{题名页(与博士答辩委员会名单)}
对应\cs{chapters}\cs{cover2.tex}文件。\textbf{密级、论文中、英文标题，作者姓名、学号，导师姓名、职称，学位类别、级别，学科专业，研究方向}信息字段编辑方法见\secref{论文基本信息字段填写}，毋需在此文件内编辑。日期会自动生成，毋需编辑。

\textbf{对于博士模板，答辩委员会名单也在此部分，请注意编辑\cs{chapters}\cs{cover2.tex}文件结尾的表格，硕士无需在意。}
...
\textbf{对于博士模板，答辩决议书也在此部分，请注意编辑\cs{chapters}\cs{Curriculum Vitae.tex}文件结尾的决议书章节，硕士无需在意。}

\subsection{独创性声明、学位论文数据集}
对应\cs{chapters}\cs{statement.tex}文件。独创性声明签名建议脱离\LaTeX{}环境后插入或手写。学位论文数据集信息字段编辑方法见\secref{论文基本信息字段填写}，毋需在此文件内编辑。
...
本模板提供了物理量名称及符号表(术语表)功能，用于整理论文中的物理常数、概念与缩写、数学符号等内容。请在\cs{chapters}\cs{nomenclature.tex}文件内编辑。基本语法有三种，分别为：
...
若有需求，可进入\cs{bjtuthesis.cls}文件内编辑术语类别，具体定义语句为：
\begin{lstlisting}[language={[LaTeX]TeX}]
    \renewcommand\nomgroup[1]{%
        \item[\hei
        \ifstrequal{#1}{A}{物理常数}{%
...
本文非常不建议手搓表格代码，强烈建议使用第三方插件如Create LaTeX tables online\cite{TabCreateLaTeXta}制作表格再拷贝进\LaTeX{}进行进一步处理，故不介绍普通表格插入命令。
